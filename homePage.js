                ...
                xhttp.onload = function() {
                    ...
                    if(getResponseText == "Record updated successfully"){
                        ...
                        if(recordedCount==totalCount){
                            ...
                            btn.onclick=function(){ProfileBtnInteraction(btn,"My_Cart.html?uname="+username)}
                            btn.click();
                        }
                    }
                }
...
function MyCartLoad(){
    ...
    errorMsg.style.width='80vw';
    errorMsg.style.display='none';
    errorMsg.style.textAlign='center';
    errorMsg.textContent="No Items Are Present in the cart."
    ...
    updateBTN.textContent="Proceed To Checkout"
    updateBTN.style.marginBottom="30px"
    updateBTN.style.marginTop="2vh";
    updateBTN.onclick=function(){ProceedToCheckoutBTN(updateBTN,errorMsg)};
    document.body.appendChild(updateBTN)
}

function ProceedToCheckoutBTN(updateBTN,errorMsg){
            ...
            for (i = 0; i < menu_cards.length; i++) {
                ...
                xhttp.onload = function() {
                    ...
                    if(getResponseText == "Record updated successfully"){
                        recordedCount+=1;
                        if(recordedCount==totalCount){
                            updateBTN.onclick=function(){ProfileBtnInteraction(updateBTN,"Checkout_page.html?uname="+username);}
                            updateBTN.click();
                        }
                    }
                }
...
function pay_securely(btn){
    ...
    let Credit_name = document.getElementById("Credit_name");
    let Credit_num = document.getElementById("Credit_number")
    let Credit_exp = document.getElementById("Credit_exp");
    let Credit_cvv = document.getElementById("Credit_cvv");
    
    let Upi_id = document.getElementById("Upi_id");

    if(radio_credit.checked){
        if(Credit_name.value=="" || Credit_num.value=="" || Credit_exp.value=="" || Credit_cvv.value==""){
            let ErrorMsg = document.getElementById("ErrorMsg");
            ErrorMsg.textContent="Please Enter Card Details."
            ErrorMsg.style.display="block";

            Credit_name.style.borderColor=(Credit_name.value=="") ? 'red' : 'transparent';
            Credit_num.style.borderColor=(Credit_num.value=="") ? 'red' : 'transparent';
            Credit_exp.style.borderColor=(Credit_exp.value=="") ? 'red' : 'transparent';
            Credit_cvv.style.borderColor=(Credit_cvv.value=="") ? 'red' : 'transparent';

            document.getElementById("button_click_please").onclick=function(){pay_securely(document.getElementById("button_click_please"))}
            return;
        }
        payment_method="Credit/Debit Card"
    }
        ...
        if(Upi_id.value==""){
            let ErrorMsg = document.getElementById("ErrorMsg");
            ErrorMsg.textContent="Please Enter UPI Id."
            ErrorMsg.style.display="block";

            Upi_id.style.borderColor='red';
        ...
        let ErrorMsg = document.getElementById("ErrorMsg");
        ErrorMsg.textContent="Please Choose a payment Option.";
        ErrorMsg.style.display="block";
        console.log("Please Choose a payment Option");
        document.getElementById("button_click_please").onclick=function(){pay_securely(document.getElementById("button_click_please"))}
        return;
